...
let vertex =
    "#version 300 es\nuniform mat4 A;uniform vec4 D,E,F,G;uniform vec3 C;in vec4 a;in vec3 b;out vec4 j;void main(){vec3 p=b*G.y;vec4 o=vec4(a.xyz+p*a.w,1.);gl_Position=A*o;float t=a.w/G.x;gl_PointSize=mix(G.z,G.w,t);j=mix(E,F,t);j=mix(j,D,smoothstep(0.,1.,clamp(0.,1.,length(C-o.xyz)/15.)));}";
